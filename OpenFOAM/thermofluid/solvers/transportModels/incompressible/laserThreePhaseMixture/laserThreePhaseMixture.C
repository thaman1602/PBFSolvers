...
:
    ...
    phase1Name_(getPhaseName("MetalProperties")),
    ...
    rhoL_("rhoL", dimensionSet(1,-3,0,0,0,0,0),nuModel1_->viscosityProperties().lookup("rhoL")),
    rhoS_("rhoS", dimensionSet(1,-3,0,0,0,0,0),nuModel1_->viscosityProperties().lookup("rhoS")),
    ...
    )
{
    ...
    Ts_ = dimensionedScalar("Ts",dimensionSet(0,0,0,1,0,0,0),thermalProperties.subDict("metalProperties").lookup("Ts"));
    Tl_ = dimensionedScalar("Tl",dimensionSet(0,0,0,1,0,0,0),thermalProperties.subDict("metalProperties").lookup("Tl"));
    calcNu();
}
...
tmp<volScalarField> threePhaseMixture::mu() const
{
    ...
    (
        ...
        (
            ...
            limitedAlphaM*((limitedgT*rhoL_)+((scalar(1)-limitedgT)*rhoS_))*nuModel1_->nu()
...
tmp<surfaceScalarField> threePhaseMixture::muf() const
{
    ...
    (
        ...
        (
            ...
            alphaMf*((gTf*rhoL_)+((scalar(1)-gTf)*rhoS_))*fvc::interpolate(nuModel1_->nu())
...
bool threePhaseMixture::read()
{
    if (transportModel::read())
    {
        ...
        {
            nuModel1_->viscosityProperties().lookup("rhoS") >> rhoS_;
